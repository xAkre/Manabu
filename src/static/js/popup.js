...
const createPopup = (type, message) => {
    ...
    container.className =
        'relative flex w-full items-center gap-2 rounded-sm border-l-2 bg-slate-50 p-2 transition-all duration-1000 ease-in-out dark:bg-slate-900';
    ...
    */
    ...
    let { svgElement } =
        type === 'success'
            ? new Tick('h-7 aspect-square')
            : new XCircle('h-7 aspect-square');
    if (type === 'success') {
    ...
    errorTitle.className = 'text-sm text-slate-900 dark:text-slate-100';
