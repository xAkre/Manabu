...
def test_can_get_add_category_page(flask_app, form_data_factory) -> None:
    """
...
@pytest.mark.usefixtures("set_temporary_database")
def test_can_add_category(flask_app, form_data_factory) -> None:
    """
    ...
    with flask_app.test_request_context():
        ...
        response = test_client.post(url_for("categories.create"), data=form_data)
...
@pytest.mark.usefixtures("set_temporary_database")
def test_cant_add_category_with_invalid_color(flask_app, form_data_factory) -> None:
    """
    ...
    with flask_app.test_request_context():
        _register_and_login_test_user(flask_app, test_client, form_data_factory)
        form_data = form_data_factory({"name": "work", "color": "#FFFFF"})
        response = test_client.post(url_for("categories.create"), data=form_data)
...
def _register_and_login_test_user(
    ...
    register_form_data = form_data_factory(
        {
            "username": "username",
            "email": "email@email.com",
            "password": "Password_123",
    ...
    login_form_data = form_data_factory(
