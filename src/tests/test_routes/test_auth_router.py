...
def test_can_log_in_with_email(flask_app) -> None:
    ...
    with flask_app.app_context(), flask_app.test_request_context(), test_client:
        ...
        response = test_client.post(
            url_for("auth.login"),
            data={"username_or_email": "email@email.com", "password": "password"},
        )
        user = d_session.execute(
            select(User).where(User.email == "email@email.com")
        ).scalar()
...
def test_cannot_log_in_with_a_username_or_email_that_does_not_exist(flask_app) -> None:
    ...
    with flask_app.app_context(), flask_app.test_request_context(), test_client:
        response = test_client.post(
            url_for("auth.login"),
            data={"username_or_email": "does_not_exist", "password": "password"},
        )
        assert response.status_code == HTTPStatus.BAD_REQUEST
...
def test_cannot_log_in_with_a_correct_username_but_incorrect_password(
    ...
    with flask_app.app_context(), flask_app.test_request_context(), test_client:
        ...
        response = test_client.post(
            url_for("auth.login"),
            data={"username_or_email": "username", "password": "incorrect_password"},
        )
        assert response.status_code == HTTPStatus.BAD_REQUEST
